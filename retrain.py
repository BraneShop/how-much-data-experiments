...
# Futher note: I changed this file to deal with a few quirks of the
# experiments. (Noon, 2018).
...
def create_image_lists(image_dir, testing_percentage, validation_percentage):
  ...
  for sub_dir in sub_dirs:
    ...
    for file_name in file_list:
      ...
      hash_name_hashed = hashlib.sha1(tf.compat.as_bytes(hash_name + FLAGS.hash_salt)).hexdigest()
...
def main(_):
  ...
  tf.logging.set_verbosity(levels[FLAGS.log_level])
  ...
  with tf.Session(graph=graph) as sess:
    ...
    for i in range(FLAGS.how_many_training_steps):
      ...
      if (i % FLAGS.eval_step_interval) == 0 or is_last_step:
        ...
        train_data.append({"step": i, "loss": cross_entropy_value, "accuracy": train_accuracy})
        ...
        if FLAGS.validation_percentage > 0:
            ...
            # with the `merged` op.
            validation_summary, cross_entropy_value, validation_accuracy = sess.run(
                [merged, cross_entropy, evaluation_step],
            ...
            val_data.append({"step": i, "loss": cross_entropy_value, "accuracy": validation_accuracy})
...
if __name__ == '__main__':
  ...
  parser.add_argument(
      ...
      type=int,
      default=2,
      help='0 = DEBUG, 1 = INFO, 2 = WARN, 3 = ERROR, 4 = FATAL'
  )
  parser.add_argument(
      '--summaries_dir',
      type=str,
